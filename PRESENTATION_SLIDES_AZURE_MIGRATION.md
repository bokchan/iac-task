...
- Constraint: CDKTF is sunset, so long-term direction must change.
- Objective: deliver a working migration.
...
- Change of cloud provider to Azure.
...
> Does the Azure architecture meet the future needs?
...
- GitHub Actions for Terraform validate and Azure-authenticated plan on PRs and main
...
| Deployment           | Terraform validate and plan run on PRs and main | Add protected environment gates, image build/push, and automated apply |
...
```
# Check containerapp logs:
az containerapp logs show -n iac-task-dev-webapp -g iac-task-dev-rg
```
